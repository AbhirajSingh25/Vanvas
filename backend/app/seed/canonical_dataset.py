"""
VANVAS Canonical Dataset for Seeding and Testing
Defines all 25 Curated Canonical Destinations, Verified Places, Hotels, and Rentals.
Self-contained, production-ready dataset.
"""

CANONICAL_25_DESTINATIONS = [   {   'altitude_meters': 2050,
        'best_time_to_visit': 'October to June',
        'description': 'Nestled in the Beas River Valley, Manali blends rustic Himalayan charm with vibrant café '
                       'culture and gateway routes to high altitude passes.',
        'description_en': 'Nestled in the Beas River Valley, Manali blends rustic Himalayan charm with vibrant café '
                          'culture and gateway routes to high altitude passes.',
        'description_hi': 'ब्यास नदी घाटी में बसा मनाली पारंपरिक हिमालयी संस्कृति और जीवंत कैफ़े जीवन का अनूठा संगम '
                          'प्रस्तुत करता है।',
        'hero_artwork': '/artworks/manali.jpg',
        'hero_image': '/images/destinations/manali/hero.jpg',
        'hero_photo': '/images/destinations/manali/hero.jpg',
        'hindi_name': 'मनाली',
        'is_featured': True,
        'latitude': 32.2396,
        'longitude': 77.1887,
        'name': 'Manali',
        'name_en': 'Manali',
        'name_hi': 'मनाली',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Himalayan',
        'slug': 'manali',
        'state': 'Himachal Pradesh',
        'subtitle_en': 'Pine-scented mountain air, riverside stone cafés, and high alpine trails.',
        'subtitle_hi': 'चीड़ की खुशबू वाली हवा, नदी किनारे कैफ़े और अल्पाइन रास्ते।',
        'tagline': 'Pine-scented mountain air, riverside stone cafés, and high alpine trails.',
        'trek_available': True,
        'weather_type': 'Alpine Mist / Cool'},
    {   'altitude_meters': 372,
        'best_time_to_visit': 'September to May',
        'description': 'The yoga capital of the world resting on the banks of the sacred Ganges, where jungle serenity '
                       'meets world-class river rafting and sunset aartis.',
        'description_en': 'The yoga capital of the world resting on the banks of the sacred Ganges, where jungle '
                          'serenity meets world-class river rafting and sunset aartis.',
        'description_hi': 'पवित्र गंगा के तट पर स्थित योग और अध्यात्म की वैश्विक राजधानी, जहाँ संध्या आरती और '
                          'प्राकृतिक शांति का अद्भुत अनुभव मिलता है।',
        'hero_artwork': '/artworks/rishikesh.jpg',
        'hero_image': '/images/destinations/rishikesh/hero.jpg',
        'hero_photo': '/images/destinations/rishikesh/hero.jpg',
        'hindi_name': 'ऋषिकेश',
        'is_featured': True,
        'latitude': 30.0869,
        'longitude': 78.2676,
        'name': 'Rishikesh',
        'name_en': 'Rishikesh',
        'name_hi': 'ऋषिकेश',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Himalayan Foothills',
        'slug': 'rishikesh',
        'state': 'Uttarakhand',
        'subtitle_en': 'Turquoise Ganga currents, cliffside meditation, and rapid adventures.',
        'subtitle_hi': 'नीली गंगा की धारा, योग साधना और रोमांचक रिवर राफ़्टिंग।',
        'tagline': 'Turquoise Ganga currents, cliffside meditation, and rapid adventures.',
        'trek_available': True,
        'weather_type': 'Pleasant / River Breeze'},
    {   'altitude_meters': 4000,
        'best_time_to_visit': 'April to November',
        'description': 'An iconic Garhwal Himalayan trail ascending from the meadows of Chopta (2,680m) through alpine '
                       'rhododendrons to the ancient stone Tungnath Temple (3,680m) and continuing 1.5 km to the '
                       'Chandrashila Summit (4,000m) with sweeping views of Chaukhamba, Trishul, and Nanda Devi.',
        'description_en': 'An iconic Garhwal Himalayan trail ascending from the meadows of Chopta (2,680m) through '
                          'alpine rhododendrons to the ancient stone Tungnath Temple (3,680m) and continuing 1.5 km to '
                          'the Chandrashila Summit (4,000m) with sweeping views of Chaukhamba, Trishul, and Nanda '
                          'Devi.',
        'description_hi': 'चोपता के मखमली बुग्यालों से शुरू होकर प्राचीन तुंगनाथ मंदिर और 4000 मीटर ऊँचे चंद्रशिला '
                          'शिखर तक जाने वाला अद्वितीय हिमालयी ट्रेक।',
        'hero_artwork': '/artworks/fallback_himalayan.jpg',
        'hero_image': '/images/destinations/tungnath-chandrashila/hero.jpg',
        'hero_photo': '/images/destinations/tungnath-chandrashila/hero.jpg',
        'hindi_name': 'तुंगनाथ–चंद्रशिला',
        'is_featured': True,
        'latitude': 30.4886,
        'longitude': 79.2173,
        'name': 'Tungnath–Chandrashila Trek',
        'name_en': 'Tungnath–Chandrashila',
        'name_hi': 'तुंगनाथ–चंद्रशिला',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Garhwal Himalayas',
        'slug': 'tungnath-chandrashila',
        'state': 'Uttarakhand',
        'subtitle_en': "World's highest Shiva shrine, alpine rhododendron bugyals, and a 360° summit over Chaukhamba.",
        'subtitle_hi': 'विश्व का सबसे ऊँचा शिव मंदिर, बुरांश के बुग्याल और चौखंभा का 360° विहंगम दृश्य।',
        'tagline': "World's highest Shiva shrine, alpine rhododendron bugyals, and a 360° summit over Chaukhamba.",
        'trek_available': True,
        'weather_type': 'High Alpine Crisp Air'},
    {   'altitude_meters': 1400,
        'best_time_to_visit': 'Round the Year',
        'description': 'Established in 1962 by the revered mystic Neem Karoli Baba, Kainchi Dham sits between two '
                       'scenic mountain hills in Nainital district. Renowned worldwide as a sanctuary of immense '
                       'spiritual peace, chanting, and selfless service.',
        'description_en': 'Established in 1962 by the revered mystic Neem Karoli Baba, Kainchi Dham sits between two '
                          'scenic mountain hills in Nainital district. Renowned worldwide as a sanctuary of immense '
                          'spiritual peace, chanting, and selfless service.',
        'description_hi': '1962 में नीम करौली बाबा द्वारा स्थापित कैंची धाम दो सुरम्य पहाड़ियों के बीच स्थित एक परम '
                          'पावन आध्यात्मिक आश्रम है।',
        'hero_artwork': '/artworks/kainchi-dham.jpg',
        'hero_image': '/images/destinations/kainchi-dham/hero.jpg',
        'hero_photo': '/images/destinations/kainchi-dham/hero.jpg',
        'hindi_name': 'कैंची धाम',
        'is_featured': True,
        'latitude': 29.4239,
        'longitude': 79.5165,
        'name': 'Kainchi Dham',
        'name_en': 'Kainchi Dham',
        'name_hi': 'कैंची धाम',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Kumaon Hills',
        'slug': 'kainchi-dham',
        'state': 'Uttarakhand',
        'subtitle_en': "Neem Karoli Baba's sacred riverside ashram nestled in pine-scented Kumaoni valleys.",
        'subtitle_hi': 'नीम करौली बाबा का पवित्र आश्रम, शांत पहाड़ी नदी और देवदार के घने जंगल।',
        'tagline': "Neem Karoli Baba's sacred riverside ashram nestled in pine-scented Kumaoni valleys.",
        'trek_available': False,
        'weather_type': 'Pleasant Mountain River Breeze'},
    {   'altitude_meters': 1580,
        'best_time_to_visit': 'March to June & Sept to Nov',
        'description': 'A tranquil haven in Parvati Valley famous for Israeli bakeries, pine-forested riverside hikes '
                       'to Chalal and Tosh, and unmatched mountain peace.',
        'description_en': 'A tranquil haven in Parvati Valley famous for Israeli bakeries, pine-forested riverside '
                          'hikes to Chalal and Tosh, and unmatched mountain peace.',
        'description_hi': 'पार्वती घाटी का एक शांत कोना, जहाँ कल-कल बहती नदी, देवदार के जंगल और सुकून भरे रास्ते मिलते '
                          'हैं।',
        'hero_artwork': '/artworks/kasol.jpg',
        'hero_image': '/images/destinations/kasol/hero.jpg',
        'hero_photo': '/images/destinations/kasol/hero.jpg',
        'hindi_name': 'कसोल',
        'is_featured': True,
        'latitude': 32.01,
        'longitude': 77.315,
        'name': 'Kasol',
        'name_en': 'Kasol',
        'name_hi': 'कसोल',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Parvati Valley',
        'slug': 'kasol',
        'state': 'Himachal Pradesh',
        'subtitle_en': 'Mystic deodar canopies, roaring emerald waters, and bohemian trails.',
        'subtitle_hi': 'देवदार के रहस्यमयी जंगल, पार्वती नदी और खूबसूरत पहाड़ी रास्ते।',
        'tagline': 'Mystic deodar canopies, roaring emerald waters, and bohemian trails.',
        'trek_available': True,
        'weather_type': 'Crisp Mountain Mist'},
    {   'altitude_meters': 1457,
        'best_time_to_visit': 'September to June',
        'description': 'Home of the Dalai Lama, surrounded by cedar forests and dramatic snow-capped peaks with '
                       'authentic momos and serene monasteries.',
        'description_en': 'Home of the Dalai Lama, surrounded by cedar forests and dramatic snow-capped peaks with '
                          'authentic momos and serene monasteries.',
        'description_hi': 'दलाई लामा का निवास स्थल, देवदार के वनों और हिमाच्छादित धौलाधार चोटियों से घिरा तिब्बती '
                          'संस्कृति का केंद्र।',
        'hero_artwork': '/artworks/dharamshala.jpg',
        'hero_image': '/images/destinations/dharamshala/hero.jpg',
        'hero_photo': '/images/destinations/dharamshala/hero.jpg',
        'hindi_name': 'धर्मशाला',
        'is_featured': True,
        'latitude': 32.219,
        'longitude': 76.3234,
        'name': 'Dharamshala & McLeod Ganj',
        'name_en': 'Dharamshala',
        'name_hi': 'धर्मशाला',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Kangra Valley',
        'slug': 'dharamshala',
        'state': 'Himachal Pradesh',
        'subtitle_en': 'Prayer flags in the mist, Tibetan heritage, and the mighty Dhauladhar ridge.',
        'subtitle_hi': 'धुंध में लहराते प्रार्थना ध्वज, तिब्बती संस्कृति और धौलाधार की विशाल पर्वतमाला।',
        'tagline': 'Prayer flags in the mist, Tibetan heritage, and the mighty Dhauladhar ridge.',
        'trek_available': True,
        'weather_type': 'Misty Cedar Air'},
    {   'altitude_meters': 10,
        'best_time_to_visit': 'November to April',
        'description': 'Beyond the crowded tourist strips lie sleepy riverside villages, historic Latin quarters, '
                       'vibrant night flea markets, and tranquil cliff beaches.',
        'description_en': 'Beyond the crowded tourist strips lie sleepy riverside villages, historic Latin quarters, '
                          'vibrant night flea markets, and tranquil cliff beaches.',
        'description_hi': 'अरब सागर के तट पर बसा सुरम्य प्रदेश, जो अपने पुर्तगाली स्थापत्य, नारियल के कुंजों और शांत '
                          'तटों के लिए प्रसिद्ध है।',
        'hero_artwork': '/artworks/goa.jpg',
        'hero_image': '/images/destinations/goa/hero.jpg',
        'hero_photo': '/images/destinations/goa/hero.jpg',
        'hindi_name': 'गोवा',
        'is_featured': True,
        'latitude': 15.2993,
        'longitude': 74.124,
        'name': 'Goa',
        'name_en': 'Goa',
        'name_hi': 'गोवा',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Coastal Western Ghats',
        'slug': 'goa',
        'state': 'Goa',
        'subtitle_en': 'Golden palms, Portuguese villas, beach shack sunsets, and spice farms.',
        'subtitle_hi': 'सुनहरी धूप, पुर्तगाली विला, सूर्यास्त और शांत समुद्री तट।',
        'tagline': 'Golden palms, Portuguese villas, beach shack sunsets, and spice farms.',
        'trek_available': False,
        'weather_type': 'Warm Coastal Breeze'},
    {   'altitude_meters': 431,
        'best_time_to_visit': 'October to March',
        'description': 'The Pink City where regal hill forts overlook bustling bazaars full of blue pottery, '
                       'block-printed fabrics, and royal Rajasthani delicacies.',
        'description_en': 'The Pink City where regal hill forts overlook bustling bazaars full of blue pottery, '
                          'block-printed fabrics, and royal Rajasthani delicacies.',
        'description_hi': 'राजस्थान की ऐतिहासिक राजधानी, जहाँ आमेर का किला, हवा महल और पारंपरिक बाज़ार राजपूताना वैभव '
                          'की गवाही देते हैं।',
        'hero_artwork': '/artworks/jaipur.jpg',
        'hero_image': '/images/destinations/jaipur/hero.jpg',
        'hero_photo': '/images/destinations/jaipur/hero.jpg',
        'hindi_name': 'जयपुर',
        'is_featured': True,
        'latitude': 26.9124,
        'longitude': 75.7873,
        'name': 'Jaipur',
        'name_en': 'Jaipur',
        'name_hi': 'जयपुर',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Royal Heritage',
        'slug': 'jaipur',
        'state': 'Rajasthan',
        'subtitle_en': 'Terracotta ramparts, historic havelis, rich kachoris, and artisan crafts.',
        'subtitle_hi': 'गुलाबी नगर के भव्य किले, हवेलियाँ और पारंपरिक राजस्थानी स्वाद।',
        'tagline': 'Terracotta ramparts, historic havelis, rich kachoris, and artisan crafts.',
        'trek_available': False,
        'weather_type': 'Dry Heritage Warmth'},
    {   'altitude_meters': 224,
        'best_time_to_visit': 'Round the Year',
        'description': 'Located along NH-44 in Sonipat district, Murthal is the capital of north Indian road trips, '
                       'renowned for 24/7 legendary dhabas like Amrik Sukhdev and Haveli.',
        'description_en': 'Located along NH-44 in Sonipat district, Murthal is the capital of north Indian road trips, '
                          'renowned for 24/7 legendary dhabas like Amrik Sukhdev and Haveli.',
        'description_hi': 'सोनीपत में स्थित मुरथल दिल्ली-एनसीआर का सबसे लोकप्रिय हाइवे डेस्टिनेशन है, जहाँ के पराठे '
                          'देश भर में प्रसिद्ध हैं।',
        'hero_artwork': '/artworks/murthal.jpg',
        'hero_image': '/images/destinations/murthal/hero.jpg',
        'hero_photo': '/images/destinations/murthal/hero.jpg',
        'hindi_name': 'मुरथल',
        'is_featured': True,
        'latitude': 29.0289,
        'longitude': 77.0707,
        'name': 'Murthal (GT Road)',
        'name_en': 'Murthal',
        'name_hi': 'मुरथल',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'GT Road Corridor',
        'slug': 'murthal',
        'state': 'Haryana',
        'subtitle_en': 'Hot tandoori parathas with white butter, clay tandoors, and legendary GT Road highway culture.',
        'subtitle_hi': 'गरमा-गरम तंदूरी पराठे, मक्खन और प्रसिद्ध जीटी रोड ढाबा संस्कृति।',
        'tagline': 'Hot tandoori parathas with white butter, clay tandoors, and legendary GT Road highway culture.',
        'trek_available': False,
        'weather_type': 'Highway Breeze'},
    {   'altitude_meters': 171,
        'best_time_to_visit': 'October to March',
        'description': 'Home to the Taj Mahal and Agra Fort, Agra combines world-renowned Mughal sandstone '
                       'architecture with bustling petha sweet lanes and riverside promenades.',
        'description_en': 'Home to the Taj Mahal and Agra Fort, Agra combines world-renowned Mughal sandstone '
                          'architecture with bustling petha sweet lanes and riverside promenades.',
        'description_hi': 'यमुना तट पर स्थित ऐतिहासिक मुग़ल राजधानी, जहाँ विश्व प्रसिद्ध ताजमहल, आगरा का किला और '
                          'फतेहपुर सीकरी स्थित हैं।',
        'hero_artwork': '/artworks/agra.jpg',
        'hero_image': '/images/destinations/agra/hero.jpg',
        'hero_photo': '/images/destinations/agra/hero.jpg',
        'hindi_name': 'आगरा',
        'is_featured': True,
        'latitude': 27.1767,
        'longitude': 78.0081,
        'name': 'Agra',
        'name_en': 'Agra',
        'name_hi': 'आगरा',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Yamuna Heritage Corridor',
        'slug': 'agra',
        'state': 'Uttar Pradesh',
        'subtitle_en': 'The timeless marble poetry of the Taj Mahal, Agra Fort, and rich Yamuna river sunsets.',
        'subtitle_hi': 'ताजमहल का श्वेत संगमरमर, आगरा का लाल किला और यमुना का विहंगम दृश्य।',
        'tagline': 'The timeless marble poetry of the Taj Mahal, Agra Fort, and rich Yamuna river sunsets.',
        'trek_available': False,
        'weather_type': 'Pleasant Heritage Winter'},
    {   'altitude_meters': 178,
        'best_time_to_visit': 'September to March',
        'description': 'The divine heartland of Braj Bhoomi, featuring Prem Mandir, Shri Krishna Janmabhoomi, Banke '
                       'Bihari Temple, and holy evening ghat aartis on the Yamuna.',
        'description_en': 'The divine heartland of Braj Bhoomi, featuring Prem Mandir, Shri Krishna Janmabhoomi, Banke '
                          'Bihari Temple, and holy evening ghat aartis on the Yamuna.',
        'description_hi': 'भगवान श्रीकृष्ण की पावन जन्मभूमि और लीला स्थली, जहाँ भक्ति, भजन और यमुना घाटों की अलौकिक '
                          'शांति का अनुभव होता है।',
        'hero_artwork': '/artworks/mathura-vrindavan.jpg',
        'hero_image': '/images/destinations/mathura-vrindavan/hero.jpg',
        'hero_photo': '/images/destinations/mathura-vrindavan/hero.jpg',
        'hindi_name': 'मथुरा और वृन्दावन',
        'is_featured': True,
        'latitude': 27.5706,
        'longitude': 77.6599,
        'name': 'Mathura & Vrindavan',
        'name_en': 'Mathura & Vrindavan',
        'name_hi': 'मथुरा और वृन्दावन',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Braj Bhoomi',
        'slug': 'mathura-vrindavan',
        'state': 'Uttar Pradesh',
        'subtitle_en': 'Sacred Yamuna aartis, Banke Bihari blessings, and glowing white marble of Prem Mandir.',
        'subtitle_hi': 'बांके बिहारी के दर्शन, यमुना आरती और प्रेम मंदिर की भव्य छटा।',
        'tagline': 'Sacred Yamuna aartis, Banke Bihari blessings, and glowing white marble of Prem Mandir.',
        'trek_available': False,
        'weather_type': 'Pleasant Temple Atmosphere'},
    {   'altitude_meters': 340,
        'best_time_to_visit': 'September to April',
        'description': 'Perched majestically on Aravalli hillocks along NH-48, Neemrana Fort Palace offers 14 stepped '
                       'levels of Rajput architecture, zip-lining, and sunset views.',
        'description_en': 'Perched majestically on Aravalli hillocks along NH-48, Neemrana Fort Palace offers 14 '
                          'stepped levels of Rajput architecture, zip-lining, and sunset views.',
        'description_hi': 'दिल्ली-जयपुर राजमार्ग पर अरावली की पहाड़ियों में स्थित 14 स्तरों वाला ऐतिहासिक किला जो अपनी '
                          'राजसी वास्तुकला के लिए विख्यात है।',
        'hero_artwork': '/artworks/neemrana.jpg',
        'hero_image': '/images/destinations/neemrana/hero.jpg',
        'hero_photo': '/images/destinations/neemrana/hero.jpg',
        'hindi_name': 'नीमराना',
        'is_featured': True,
        'latitude': 27.9942,
        'longitude': 76.3869,
        'name': 'Neemrana Fort',
        'name_en': 'Neemrana',
        'name_hi': 'नीमराना',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Aravalli Heritage',
        'slug': 'neemrana',
        'state': 'Rajasthan',
        'subtitle_en': '15th-century stepped fortress palace carved into Aravalli cliffs along the Jaipur Highway.',
        'subtitle_hi': 'अरावली की पहाड़ियों पर बना 15वीं सदी का सीढ़ीदार राजसी किला महल।',
        'tagline': '15th-century stepped fortress palace carved into Aravalli cliffs along the Jaipur Highway.',
        'trek_available': False,
        'weather_type': 'Crisp Heritage Breeze'},
    {   'altitude_meters': 220,
        'best_time_to_visit': 'October to March',
        'description': "One of Haryana's largest natural lakes, resting at the base of the Aravalli hills near Sohna, "
                       'perfect for boating, rock climbing, and weekend picnics.',
        'description_en': "One of Haryana's largest natural lakes, resting at the base of the Aravalli hills near "
                          'Sohna, perfect for boating, rock climbing, and weekend picnics.',
        'description_hi': 'अरावली की पहाड़ियों की तलहटी में स्थित प्राकृतिक झील, जो दिल्ली-एनसीआर के यात्रियों के लिए '
                          'नौकायन और आउटडोर पिकनिक का आदर्श स्थान है।',
        'hero_artwork': '/artworks/damdama-sohna.jpg',
        'hero_image': '/images/destinations/damdama-sohna/hero.jpg',
        'hero_photo': '/images/destinations/damdama-sohna/hero.jpg',
        'hindi_name': 'दमदमा और सोहना',
        'is_featured': True,
        'latitude': 28.3075,
        'longitude': 77.0617,
        'name': 'Damdama & Sohna',
        'name_en': 'Damdama & Sohna',
        'name_hi': 'दमदमा और सोहना',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Aravalli Foothills',
        'slug': 'damdama-sohna',
        'state': 'Haryana',
        'subtitle_en': 'Tranquil lake waters framed by rugged Aravalli rocky ridges, rowing boats, and rustic trails.',
        'subtitle_hi': 'अरावली की गोद में बसी शांत झील, नौका विहार और प्राकृतिक पहाड़ियाँ।',
        'tagline': 'Tranquil lake waters framed by rugged Aravalli rocky ridges, rowing boats, and rustic trails.',
        'trek_available': False,
        'weather_type': 'Lakeside Breeze'},
    {   'altitude_meters': 270,
        'best_time_to_visit': 'October to March',
        'description': 'Surrounded by the dense Aravalli mountain hills, Siliserh Lake Palace was built in 1845 as a '
                       'royal hunting lodge, offering boat rides and serene mountain vistas.',
        'description_en': 'Surrounded by the dense Aravalli mountain hills, Siliserh Lake Palace was built in 1845 as '
                          'a royal hunting lodge, offering boat rides and serene mountain vistas.',
        'description_hi': '1845 में निर्मित सिलीसेढ़ झील पैलेस अरावली की वादियों में स्थित एक शांत व सुंदर पर्यटन स्थल '
                          'है।',
        'hero_artwork': '/artworks/alwar-siliserh.jpg',
        'hero_image': '/images/destinations/alwar-siliserh/hero.jpg',
        'hero_photo': '/images/destinations/alwar-siliserh/hero.jpg',
        'hindi_name': 'अलवर और सिलीसेढ़',
        'is_featured': True,
        'latitude': 27.5342,
        'longitude': 76.5411,
        'name': 'Alwar & Siliserh Lake',
        'name_en': 'Alwar & Siliserh',
        'name_hi': 'अलवर और सिलीसेढ़',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Mewat & Aravalli Hills',
        'slug': 'alwar-siliserh',
        'state': 'Rajasthan',
        'subtitle_en': 'Historic 19th-century royal lake palace, tranquil waters, and Aravalli mountain reflections.',
        'subtitle_hi': 'सिलीसेढ़ झील का राजसी पैलेस, नीली जलधारा और अरावली की पहाड़ियाँ।',
        'tagline': 'Historic 19th-century royal lake palace, tranquil waters, and Aravalli mountain reflections.',
        'trek_available': False,
        'weather_type': 'Pleasant Hill Breeze'},
    {   'altitude_meters': 420,
        'best_time_to_visit': 'October to April',
        'description': 'Combines the thrill of tiger safari in Sariska National Park with the architectural mystery '
                       'and scenic stone ramparts of the 17th-century Bhangarh Fort ruins.',
        'description_en': 'Combines the thrill of tiger safari in Sariska National Park with the architectural mystery '
                          'and scenic stone ramparts of the 17th-century Bhangarh Fort ruins.',
        'description_hi': 'अरावली की घनी वादियों में स्थित सरिस्का टाइगर रिजर्व और 17वीं सदी का रहस्यमयी भानगढ़ किला।',
        'hero_artwork': '/artworks/sariska-bhangarh.jpg',
        'hero_image': '/images/destinations/sariska-bhangarh/hero.jpg',
        'hero_photo': '/images/destinations/sariska-bhangarh/hero.jpg',
        'hindi_name': 'सरिस्का और भानगढ़',
        'is_featured': True,
        'latitude': 27.3292,
        'longitude': 76.4389,
        'name': 'Sariska & Bhangarh',
        'name_en': 'Sariska & Bhangarh',
        'name_hi': 'सरिस्का और भानगढ़',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Aravalli Wilderness',
        'slug': 'sariska-bhangarh',
        'state': 'Rajasthan',
        'subtitle_en': 'Dense dry deciduous forest wildlife, tiger territory, and legendary ancient stone ruins of '
                       'Bhangarh.',
        'subtitle_hi': 'सरिस्का के घने जंगल, बाघ अभयारण्य और भानगढ़ के ऐतिहासिक रहस्यमयी खंडहर।',
        'tagline': 'Dense dry deciduous forest wildlife, tiger territory, and legendary ancient stone ruins of '
                   'Bhangarh.',
        'trek_available': False,
        'weather_type': 'Dry Forest Air'},
    {   'altitude_meters': 640,
        'best_time_to_visit': 'Round the Year',
        'description': 'Nestled in the Doon Valley between the Ganga and Yamuna, Dehradun offers historic bakery '
                       "trails along Rajpur Road, Robber's Cave, and gateway paths to Mussoorie.",
        'description_en': 'Nestled in the Doon Valley between the Ganga and Yamuna, Dehradun offers historic bakery '
                          "trails along Rajpur Road, Robber's Cave, and gateway paths to Mussoorie.",
        'description_hi': 'गंगा और यमुना के मध्य दून घाटी में बसा शांत शहर, जो अपनी ऐतिहासिक बेकरियों, गुच्चूपानी और '
                          'सुहावने मौसम के लिए जाना जाता है।',
        'hero_artwork': '/artworks/dehradun.jpg',
        'hero_image': '/images/destinations/dehradun/hero.jpg',
        'hero_photo': '/images/destinations/dehradun/hero.jpg',
        'hindi_name': 'देहरादून',
        'is_featured': True,
        'latitude': 30.3165,
        'longitude': 78.0322,
        'name': 'Dehradun (Rajpur Road)',
        'name_en': 'Dehradun',
        'name_hi': 'देहरादून',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Doon Valley Foothills',
        'slug': 'dehradun',
        'state': 'Uttarakhand',
        'subtitle_en': 'Sal-forested foothills, historic Rajpur Road bakeries, and cool Himalayan mountain streams.',
        'subtitle_hi': 'दून घाटी के साल वन, राजपुर रोड की ऐतिहासिक बेकरियाँ और ठंडी नदियाँ।',
        'tagline': 'Sal-forested foothills, historic Rajpur Road bakeries, and cool Himalayan mountain streams.',
        'trek_available': True,
        'weather_type': 'Pleasant Valley Mist'},
    {   'altitude_meters': 321,
        'best_time_to_visit': 'October to March',
        'description': 'Designed by Le Corbusier at the base of the Shivalik hills, Chandigarh is celebrated for its '
                       'modernist urban planning, Sukhna Lake sunsets, and the iconic Rock Garden.',
        'description_en': 'Designed by Le Corbusier at the base of the Shivalik hills, Chandigarh is celebrated for '
                          'its modernist urban planning, Sukhna Lake sunsets, and the iconic Rock Garden.',
        'description_hi': 'शिवालिक की तलहटी में ली कोर्बुज़िए द्वारा डिज़ाइन किया गया सुंदर शहर, जो सुखना झील और रॉक '
                          'गार्डन के लिए प्रसिद्ध है।',
        'hero_artwork': '/artworks/chandigarh.jpg',
        'hero_image': '/images/destinations/chandigarh/hero.jpg',
        'hero_photo': '/images/destinations/chandigarh/hero.jpg',
        'hindi_name': 'चंडीगढ़',
        'is_featured': True,
        'latitude': 30.7333,
        'longitude': 76.7794,
        'name': 'Chandigarh',
        'name_en': 'Chandigarh',
        'name_hi': 'चंडीगढ़',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Shivalik Plains',
        'slug': 'chandigarh',
        'state': 'Punjab / Haryana',
        'subtitle_en': "Sukhna Lake morning reflections, Nek Chand's Rock Garden, and tree-lined modernist boulevards.",
        'subtitle_hi': 'सुखना झील की शांत लहरें, रॉक गार्डन की कलाकृतियाँ और शिवालिक की तलहटी।',
        'tagline': "Sukhna Lake morning reflections, Nek Chand's Rock Garden, and tree-lined modernist boulevards.",
        'trek_available': False,
        'weather_type': 'Crisp Pleasant Air'},
    {   'altitude_meters': 1220,
        'best_time_to_visit': 'September to April',
        'description': "Haryana's only hill station, nestled in the Shivalik ranges, featuring the sacred twin lakes "
                       'of Tikkar Taal, pine forests, and scenic viewpoints.',
        'description_en': "Haryana's only hill station, nestled in the Shivalik ranges, featuring the sacred twin "
                          'lakes of Tikkar Taal, pine forests, and scenic viewpoints.',
        'description_hi': 'हरियाणा का एकमात्र हिल स्टेशन, जो शिवालिक पहाड़ियों, टिक्कर ताल झील और शांत देवदार-चीड़ के '
                          'वनों से आच्छादित है।',
        'hero_artwork': '/artworks/morni-hills.jpg',
        'hero_image': '/images/destinations/morni-hills/hero.jpg',
        'hero_photo': '/images/destinations/morni-hills/hero.jpg',
        'hindi_name': 'मोरनी हिल्स',
        'is_featured': True,
        'latitude': 30.6908,
        'longitude': 77.0869,
        'name': 'Morni Hills',
        'name_en': 'Morni Hills',
        'name_hi': 'मोरनी हिल्स',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Shivalik Hills',
        'slug': 'morni-hills',
        'state': 'Haryana',
        'subtitle_en': 'Pine-covered Shivalik mountain ridges, peaceful Tikkar Taal lake, and quiet hill roads.',
        'subtitle_hi': 'चीड़ से ढकी मोरनी पहाड़ियाँ, शांत टिक्कर ताल और सुहावनी ठंडी हवा।',
        'tagline': 'Pine-covered Shivalik mountain ridges, peaceful Tikkar Taal lake, and quiet hill roads.',
        'trek_available': True,
        'weather_type': 'Cool Mountain Air'},
    {   'altitude_meters': 1706,
        'best_time_to_visit': 'Round the Year',
        'description': 'An unspoilt Garhwal cantonment town founded in 1887, surrounded by dense blue pine forests '
                       'with serene walking trails to Tip-in-Top and Bhulla Tal.',
        'description_en': 'An unspoilt Garhwal cantonment town founded in 1887, surrounded by dense blue pine forests '
                          'with serene walking trails to Tip-in-Top and Bhulla Tal.',
        'description_hi': 'गढ़वाल राइफल्स का मुख्यालय और 1887 में स्थापित शांत छावनी नगर, जो टिप-इन-टॉप और भुल्ला ताल '
                          'के लिए प्रसिद्ध है।',
        'hero_artwork': '/artworks/lansdowne.jpg',
        'hero_image': '/images/destinations/lansdowne/hero.jpg',
        'hero_photo': '/images/destinations/lansdowne/hero.jpg',
        'hindi_name': 'लैंसडाउन',
        'is_featured': True,
        'latitude': 29.8377,
        'longitude': 78.6872,
        'name': 'Lansdowne',
        'name_en': 'Lansdowne',
        'name_hi': 'लैंसडाउन',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Garhwal Hills',
        'slug': 'lansdowne',
        'state': 'Uttarakhand',
        'subtitle_en': 'Quiet oak and blue pine ridge walks, colonial stone churches, and snow-peak panoramas.',
        'subtitle_hi': 'बाँज और नीले चीड़ के शांत वन, ऐतिहासिक गिरजाघर और हिमालयी चोटियों के दृश्य।',
        'tagline': 'Quiet oak and blue pine ridge walks, colonial stone churches, and snow-peak panoramas.',
        'trek_available': True,
        'weather_type': 'Crisp Pine Breeze'},
    {   'altitude_meters': 2005,
        'best_time_to_visit': 'March to June & Sept to Nov',
        'description': 'Perched on a horseshoe ridge overlooking the Doon Valley, offering tranquil walks along '
                       "Camel's Back Road and historic bakeries in Landour.",
        'description_en': 'Perched on a horseshoe ridge overlooking the Doon Valley, offering tranquil walks along '
                          "Camel's Back Road and historic bakeries in Landour.",
        'description_hi': 'दून घाटी के ऊपर स्थित पहाड़ों की रानी, जो अपनी औपनिवेशिक वास्तुकला, कैमल्स बैक रोड और लाल '
                          'टिब्बा के लिए प्रसिद्ध है।',
        'hero_artwork': '/artworks/mussoorie.jpg',
        'hero_image': '/images/destinations/mussoorie/hero.jpg',
        'hero_photo': '/images/destinations/mussoorie/hero.jpg',
        'hindi_name': 'मसूरी',
        'is_featured': True,
        'latitude': 30.4598,
        'longitude': 78.0644,
        'name': 'Mussoorie',
        'name_en': 'Mussoorie',
        'name_hi': 'मसूरी',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Garhwal Hills',
        'slug': 'mussoorie',
        'state': 'Uttarakhand',
        'subtitle_en': 'Queen of the Hills, colonial bookshops, winterline sunsets, and oak trails.',
        'subtitle_hi': 'पहाड़ों की रानी, विंटरलाइन का विहंगम सूर्यास्त और ओक-देवदार के रास्ते।',
        'tagline': 'Queen of the Hills, colonial bookshops, winterline sunsets, and oak trails.',
        'trek_available': True,
        'weather_type': 'Cool Mountain Mist'},
    {   'altitude_meters': 598,
        'best_time_to_visit': 'September to March',
        'description': 'The City of Lakes framed by the Aravalli Hills, offering tranquil boat rides on Lake Pichola '
                       'and authentic Mewari hospitality.',
        'description_en': 'The City of Lakes framed by the Aravalli Hills, offering tranquil boat rides on Lake '
                          'Pichola and authentic Mewari hospitality.',
        'description_hi': 'अरावली पहाड़ियों से घिरी झीलों की नगरी, जहाँ पिछोला झील और सिटी पैलेस का भव्य नजारा मिलता '
                          'है।',
        'hero_artwork': '/artworks/udaipur.jpg',
        'hero_image': '/images/destinations/udaipur/hero.jpg',
        'hero_photo': '/images/destinations/udaipur/hero.jpg',
        'hindi_name': 'उदयपुर',
        'is_featured': True,
        'latitude': 24.5854,
        'longitude': 73.7125,
        'name': 'Udaipur',
        'name_en': 'Udaipur',
        'name_hi': 'उदयपुर',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Mewar Lakes',
        'slug': 'udaipur',
        'state': 'Rajasthan',
        'subtitle_en': 'Shimmering lake waters, whitewashed palaces, and romantic rooftop evenings.',
        'subtitle_hi': 'झीलों की नगरी के श्वेत महल, पिछोला का जल और मनमोहक शामें।',
        'tagline': 'Shimmering lake waters, whitewashed palaces, and romantic rooftop evenings.',
        'trek_available': False,
        'weather_type': 'Pleasant Lake Breeze'},
    {   'altitude_meters': 80,
        'best_time_to_visit': 'October to March',
        'description': 'One of the oldest continuously inhabited cities on earth, where life, philosophy, and '
                       'spiritual devotion revolve around the sacred Ganges.',
        'description_en': 'One of the oldest continuously inhabited cities on earth, where life, philosophy, and '
                          'spiritual devotion revolve around the sacred Ganges.',
        'description_hi': 'संसार की सबसे प्राचीन नगरी, जहाँ दशाश्वमेध और अस्सी घाट पर जीवन, दर्शन और गंगा आरती का '
                          'अद्भुत संगम देखने को मिलता है।',
        'hero_artwork': '/images/destinations/varanasi/illustration.jpg',
        'hero_image': '/images/destinations/varanasi/hero.jpg',
        'hero_photo': '/images/destinations/varanasi/hero.jpg',
        'hindi_name': 'वाराणसी',
        'is_featured': True,
        'latitude': 25.3176,
        'longitude': 82.9739,
        'name': 'Varanasi',
        'name_en': 'Varanasi',
        'name_hi': 'वाराणसी',
        'nearby_available': True,
        'one_day_available': True,
        'region': 'Ganga Riverfront',
        'slug': 'varanasi',
        'state': 'Uttar Pradesh',
        'subtitle_en': 'Ancient eternal ghats, dawn boat reflections, sacred chanting, and silk lanes.',
        'subtitle_hi': 'काशी के पावन घाट, भोर की नौका सवारी और गंगा आरती का दिव्य अनुभव।',
        'tagline': 'Ancient eternal ghats, dawn boat reflections, sacred chanting, and silk lanes.',
        'trek_available': False,
        'weather_type': 'Ancient River Breeze'},
    {   'altitude_meters': 3500,
        'best_time_to_visit': 'May to October',
        'description': 'The crown of Ladakh situated in the Indus River Valley, where stark dramatic mountain terrain '
                       'meets ancient Tibetan Buddhist culture.',
        'description_en': 'The crown of Ladakh situated in the Indus River Valley, where stark dramatic mountain '
                          'terrain meets ancient Tibetan Buddhist culture.',
        'description_hi': 'सिंधु नदी घाटी में स्थित लद्दाख का हृदय, जहाँ प्राचीन थिकसे व हेमिस मठ और खारदुंग ला जैसे '
                          'ऊँचे दर्रे स्थित हैं।',
        'hero_artwork': '/artworks/fallback_himalayan.jpg',
        'hero_image': '/images/destinations/leh/hero.jpg',
        'hero_photo': '/images/destinations/leh/hero.jpg',
        'hindi_name': 'लेह',
        'is_featured': True,
        'latitude': 34.1526,
        'longitude': 77.5771,
        'name': 'Leh',
        'name_en': 'Leh',
        'name_hi': 'लेह',
        'nearby_available': True,
        'one_day_available': False,
        'region': 'Trans-Himalayan Cold Desert',
        'slug': 'leh',
        'state': 'Ladakh',
        'subtitle_en': 'Barren moonscapes, thousand-year-old gompas, and world-highest motorable passes.',
        'subtitle_hi': 'लद्दाख के विहंगम मरुस्थल, प्राचीन बौद्ध मठ और विश्व के सबसे ऊँचे दर्रे।',
        'tagline': 'Barren moonscapes, thousand-year-old gompas, and world-highest motorable passes.',
        'trek_available': True,
        'weather_type': 'High Altitude Crisp Air'},
    {   'altitude_meters': 3800,
        'best_time_to_visit': 'June to October',
        'description': 'A high-altitude desert wonderland carved by the Spiti River, renowned for century-old gompas '
                       'like Key and Dhankar, and pristine high-altitude lakes.',
        'description_en': 'A high-altitude desert wonderland carved by the Spiti River, renowned for century-old '
                          'gompas like Key and Dhankar, and pristine high-altitude lakes.',
        'description_hi': 'स्पीति नदी द्वारा तराशी गई शीत मरुस्थल घाटी, जहाँ 1000 वर्ष पुराना की मठ और चंद्रताल झील '
                          'स्थित हैं।',
        'hero_artwork': '/artworks/fallback_desert.jpg',
        'hero_image': '/images/destinations/spiti-valley/hero.jpg',
        'hero_photo': '/images/destinations/spiti-valley/hero.jpg',
        'hindi_name': 'स्पीति घाटी',
        'is_featured': True,
        'latitude': 32.2461,
        'longitude': 78.0349,
        'name': 'Spiti Valley',
        'name_en': 'Spiti Valley',
        'name_hi': 'स्पीति घाटी',
        'nearby_available': True,
        'one_day_available': False,
        'region': 'Cold Desert Valley',
        'slug': 'spiti',
        'state': 'Himachal Pradesh',
        'subtitle_en': 'The middle land between Tibet and India, cliffside monasteries, and fossil villages.',
        'subtitle_hi': 'तिब्बत और भारत के बीच का मध्य देश, की गोम्पा और जीवाश्मों के गाँव।',
        'tagline': 'The middle land between Tibet and India, cliffside monasteries, and fossil villages.',
        'trek_available': True,
        'weather_type': 'Dry Cold Moonscape'},
    {   'altitude_meters': 1600,
        'best_time_to_visit': 'September to May',
        'description': 'Perched at the confluence of three mountain streams in the Western Ghats, Munnar offers '
                       'endless green tea slopes, cool breezes, and colonial bungalows.',
        'description_en': 'Perched at the confluence of three mountain streams in the Western Ghats, Munnar offers '
                          'endless green tea slopes, cool breezes, and colonial bungalows.',
        'description_hi': 'पश्चिमी घाट की वादियों में स्थित मुन्नार हरी-भरी चाय की ढलानों, शीतल बयार और अनामुडी चोटी '
                          'के लिए प्रसिद्ध है।',
        'hero_artwork': '/artworks/fallback_valley.jpg',
        'hero_image': '/images/destinations/fallbacks/valley.jpg',
        'hero_photo': '/images/destinations/fallbacks/valley.jpg',
        'hindi_name': 'मुन्नार',
        'is_featured': True,
        'latitude': 10.0889,
        'longitude': 77.0595,
        'name': 'Munnar',
        'name_en': 'Munnar',
        'name_hi': 'मुन्नार',
        'nearby_available': True,
        'one_day_available': False,
        'region': 'Western Ghats Tea Hills',
        'slug': 'munnar',
        'state': 'Kerala',
        'subtitle_en': 'Rolling emerald tea plantations, misty mountain gaps, and cardamom forests.',
        'subtitle_hi': 'मखमली चाय के बागान, धुंध भरी पहाड़ियाँ और इलायची के महकते जंगल।',
        'tagline': 'Rolling emerald tea plantations, misty mountain gaps, and cardamom forests.',
        'trek_available': True,
        'weather_type': 'Misty Green Slopes'}]

ADDITIONAL_PLACES_BY_DEST = {   'agra': [   {   'address': 'Dharmapuri, Forest Colony, Tajganj, Agra, UP',
                    'approx_cost': 250.0,
                    'category': 'Must Visit',
                    'closing_time': '18:30',
                    'description': '17th-century UNESCO World Heritage white '
                                   'marble mausoleum built by Shah Jahan on '
                                   'the banks of the Yamuna River.',
                    'image_url': '/images/places/universal/stay.webp',
                    'is_hidden_gem': False,
                    'is_indoor': False,
                    'is_must_visit': True,
                    'latitude': 27.1751,
                    'longitude': 78.0421,
                    'name': 'Taj Mahal White Marble Monument',
                    'opening_time': '06:00',
                    'price_level': '₹₹',
                    'rating': 4.9,
                    'review_count': 5200,
                    'slug': 'taj-mahal-agra',
                    'tags': 'UNESCO,Taj Mahal,Mughal Architecture,Yamuna River',
                    'why_vanvas_recommends': 'World-famous masterpiece of '
                                             'symmetrical marble architecture '
                                             'at dawn.'},
                {   'address': 'Agra Fort, Rakabganj, Agra, UP',
                    'approx_cost': 50.0,
                    'category': 'Culture & Heritage',
                    'closing_time': '18:00',
                    'description': 'Historic red sandstone fortress residence '
                                   'of the Mughal emperors with expansive '
                                   'courtyards and direct vistas of the Taj '
                                   'Mahal.',
                    'image_url': '/images/places/universal/stay.webp',
                    'is_hidden_gem': False,
                    'is_indoor': False,
                    'is_must_visit': True,
                    'latitude': 27.1795,
                    'longitude': 78.0211,
                    'name': 'Agra Red Fort & Jahangiri Mahal',
                    'opening_time': '06:00',
                    'price_level': '₹',
                    'rating': 4.8,
                    'review_count': 2100,
                    'slug': 'agra-red-fort',
                    'tags': 'Fort,Mughal History,Architecture,Heritage',
                    'why_vanvas_recommends': 'Massive double sandstone walls '
                                             'and royal Diwan-i-Khas '
                                             'chambers.'}],
    'alwar-siliserh': [   {   'address': 'Siliserh Lake, Alwar, Rajasthan',
                              'approx_cost': 100.0,
                              'category': 'Must Visit',
                              'closing_time': '18:30',
                              'description': '1845 royal hunting lodge built '
                                             'by Maharaja Vinay Singh '
                                             'overlooking a 7-sq-km tranquil '
                                             'lake ringed by forested Aravalli '
                                             'hills.',
                              'image_url': '/images/places/universal/stay.webp',
                              'is_hidden_gem': False,
                              'is_indoor': False,
                              'is_must_visit': True,
                              'latitude': 27.5342,
                              'longitude': 76.5411,
                              'name': 'Siliserh Lake Palace & Water Promenade',
                              'opening_time': '08:30',
                              'price_level': '₹',
                              'rating': 4.7,
                              'review_count': 1640,
                              'slug': 'siliserh-lake-palace',
                              'tags': 'Lake Palace,Boating,Aravalli,Royal '
                                      'Heritage',
                              'why_vanvas_recommends': 'Scenic lakeside tea '
                                                       'with sweeping mountain '
                                                       'water views.'},
                          {   'address': 'Bala Quila Road, Alwar, Rajasthan',
                              'approx_cost': 0.0,
                              'category': 'Culture & Heritage',
                              'closing_time': '17:30',
                              'description': 'Sprawling 10th-century hilltop '
                                             'fortress extending 5 km along a '
                                             'steep Aravalli ridge, featuring '
                                             '15 large towers and 51 small '
                                             'towers.',
                              'image_url': '/images/places/universal/stay.webp',
                              'is_hidden_gem': False,
                              'is_indoor': False,
                              'is_must_visit': True,
                              'latitude': 27.575,
                              'longitude': 76.592,
                              'name': 'Bala Quila (Alwar Hill Fort)',
                              'opening_time': '06:00',
                              'price_level': 'Free',
                              'rating': 4.7,
                              'review_count': 780,
                              'slug': 'bala-quila-alwar',
                              'tags': 'Hill Fort,Aravalli,Heritage,Panoramic '
                                      'Views',
                              'why_vanvas_recommends': 'Panoramic 360-degree '
                                                       'views over Alwar city '
                                                       'and surrounding forest '
                                                       'ranges.'}],
    'chandigarh': [   {   'address': 'Sector 1, Chandigarh, Punjab/Haryana',
                          'approx_cost': 0.0,
                          'category': 'Must Visit',
                          'closing_time': '21:30',
                          'description': '3-sq-km pristine rainfed lake at the '
                                         'base of the Shivalik hills, with a 2 '
                                         'km tree-lined walking promenade, '
                                         'rowing club, and bird sanctuary.',
                          'image_url': '/images/places/universal/stay.webp',
                          'is_hidden_gem': False,
                          'is_indoor': False,
                          'is_must_visit': True,
                          'latitude': 30.7421,
                          'longitude': 76.8188,
                          'name': 'Sukhna Lake Promenade & Boating',
                          'opening_time': '05:00',
                          'price_level': 'Free',
                          'rating': 4.8,
                          'review_count': 3100,
                          'slug': 'sukhna-lake-chandigarh',
                          'tags': 'Lake,Promenade,Shivalik '
                                  'Views,Sunset,Boating',
                          'why_vanvas_recommends': 'Unbeatable morning fresh '
                                                   'air walk with views of the '
                                                   'Shivalik mountain ridge.'},
                      {   'address': 'Sector 1, Near Sukhna Lake, Chandigarh',
                          'approx_cost': 40.0,
                          'category': 'Culture & Heritage',
                          'closing_time': '19:00',
                          'description': 'Visionary 40-acre sculpture garden '
                                         'created entirely from recycled '
                                         'household ceramic tiles, broken '
                                         'bangles, electrical waste, and '
                                         'rocks.',
                          'image_url': '/images/places/universal/stay.webp',
                          'is_hidden_gem': False,
                          'is_indoor': False,
                          'is_must_visit': True,
                          'latitude': 30.7525,
                          'longitude': 76.8069,
                          'name': "Nek Chand's Rock Garden",
                          'opening_time': '09:00',
                          'price_level': '₹',
                          'rating': 4.8,
                          'review_count': 2800,
                          'slug': 'rock-garden-chandigarh',
                          'tags': 'Art,Sculpture,Rock Garden,Modernist,UNESCO',
                          'why_vanvas_recommends': "One of India's most "
                                                   'extraordinary folk art and '
                                                   'environmental monuments.'}],
    'damdama-sohna': [   {   'address': 'Damdama Lake, Near Sohna, Gurugram, '
                                        'Haryana',
                             'approx_cost': 150.0,
                             'category': 'Must Visit',
                             'closing_time': '18:00',
                             'description': 'Serene natural lake reservoir '
                                            'framed by the rocky Aravalli '
                                            'mountain ranges, popular for '
                                            'pedal boating, kayaking, and '
                                            'sunset walks.',
                             'image_url': '/images/places/universal/stay.webp',
                             'is_hidden_gem': False,
                             'is_indoor': False,
                             'is_must_visit': True,
                             'latitude': 28.3075,
                             'longitude': 77.0617,
                             'name': 'Damdama Lake Boating & Aravalli Shore',
                             'opening_time': '09:00',
                             'price_level': '₹',
                             'rating': 4.6,
                             'review_count': 1450,
                             'slug': 'damdama-lake-boating',
                             'tags': 'Lake,Boating,Aravalli,Picnic,Nature',
                             'why_vanvas_recommends': 'Tranquil water getaway '
                                                      'located just outside '
                                                      'Delhi-Gurugram.'},
                         {   'address': 'Shiva Kund, Sohna Town, Gurugram, '
                                        'Haryana',
                             'approx_cost': 0.0,
                             'category': 'Culture & Heritage',
                             'closing_time': '19:00',
                             'description': 'Historical natural hot sulphur '
                                            'water spring in Sohna believed to '
                                            'possess therapeutic mineral '
                                            'healing properties.',
                             'image_url': '/images/places/universal/stay.webp',
                             'is_hidden_gem': True,
                             'is_indoor': False,
                             'is_must_visit': False,
                             'latitude': 28.248,
                             'longitude': 77.062,
                             'name': 'Sohna Sulphur Springs & Shiva Kund',
                             'opening_time': '06:00',
                             'price_level': 'Free',
                             'rating': 4.5,
                             'review_count': 520,
                             'slug': 'sohna-sulphur-springs',
                             'tags': 'Hot Spring,Heritage,Sohna,Healing',
                             'why_vanvas_recommends': 'Centuries-old hot water '
                                                      'spring with historic '
                                                      'stone kunds.'}],
    'dehradun': [   {   'address': 'Guchhupani, Anarwala, Dehradun, '
                                   'Uttarakhand',
                        'approx_cost': 50.0,
                        'category': 'Must Visit',
                        'closing_time': '18:30',
                        'description': 'A natural 600m limestone gorge where a '
                                       'cold stream flows between dramatic '
                                       'narrow cave walls with a central '
                                       'waterfall.',
                        'image_url': '/images/places/universal/stay.webp',
                        'is_hidden_gem': False,
                        'is_indoor': False,
                        'is_must_visit': True,
                        'latitude': 30.378,
                        'longitude': 78.061,
                        'name': "Robber's Cave (Guchhupani Limestone Stream)",
                        'opening_time': '07:00',
                        'price_level': '₹',
                        'rating': 4.7,
                        'review_count': 2400,
                        'slug': 'robbers-cave-dehradun',
                        'tags': 'Cave,River Walk,Limestone Gorge,Waterfall',
                        'why_vanvas_recommends': 'Wading through cool crystal '
                                                 'clear cave waters on warm '
                                                 'afternoons.'},
                    {   'address': 'Rajpur Road, Dehradun, Uttarakhand',
                        'approx_cost': 350.0,
                        'category': 'Cafés & Bakery',
                        'closing_time': '22:30',
                        'description': 'Historic tree-shaded avenue lined with '
                                       'colonial bakeries, artisan coffee '
                                       'roasters, and serene foothill '
                                       'viewpoints.',
                        'image_url': '/images/places/universal/stay.webp',
                        'is_hidden_gem': False,
                        'is_indoor': True,
                        'is_must_visit': True,
                        'latitude': 30.345,
                        'longitude': 78.058,
                        'name': 'Rajpur Road Heritage Bakeries & Cafés',
                        'opening_time': '08:00',
                        'price_level': '₹₹',
                        'rating': 4.8,
                        'review_count': 1150,
                        'slug': 'rajpur-road-bakeries',
                        'tags': 'Bakery,Coffee,Heritage,Walking,Foothills',
                        'why_vanvas_recommends': 'Classic Doon bakery culture '
                                                 'with fresh pastries and '
                                                 'mountain coffee.'}],
    'kainchi-dham': [   {   'address': 'Bhowali-Almora Road, Kainchi, '
                                       'Uttarakhand',
                            'approx_cost': 0.0,
                            'category': 'Culture & Heritage',
                            'closing_time': '19:00',
                            'description': 'Sacred riverside ashram founded in '
                                           '1962 by Neem Karoli Baba, revered '
                                           'for peaceful prayer, chanting, and '
                                           'selfless service.',
                            'image_url': '/images/places/universal/stay.webp',
                            'is_hidden_gem': False,
                            'is_indoor': False,
                            'is_must_visit': True,
                            'latitude': 29.4241,
                            'longitude': 79.5168,
                            'name': 'Neem Karoli Baba Ashram & Samadhi Sanctum',
                            'opening_time': '06:30',
                            'price_level': 'Free',
                            'rating': 4.9,
                            'review_count': 1820,
                            'slug': 'neem-karoli-baba-ashram',
                            'tags': 'Ashram,Spiritual,Meditation,Riverside',
                            'why_vanvas_recommends': 'Immense spiritual '
                                                     'tranquility and evening '
                                                     'chanting in pine-scented '
                                                     'Kumaoni hills.'},
                        {   'address': 'Beside Kainchi Ashram, Nainital '
                                       'District',
                            'approx_cost': 0.0,
                            'category': 'Nature & Trails',
                            'closing_time': '18:30',
                            'description': 'Clear mountain river stream '
                                           'flowing beside the ashram through '
                                           'smooth granite boulders and lush '
                                           'pine forest ridges.',
                            'image_url': '/images/places/universal/stay.webp',
                            'is_hidden_gem': False,
                            'is_indoor': False,
                            'is_must_visit': True,
                            'latitude': 29.4235,
                            'longitude': 79.516,
                            'name': 'Kainchi River Stream & Granite Boulders',
                            'opening_time': '06:00',
                            'price_level': 'Free',
                            'rating': 4.8,
                            'review_count': 640,
                            'slug': 'kainchi-river-stream',
                            'tags': 'Riverside,Scenic,Pine Forests',
                            'why_vanvas_recommends': 'Tranquil walking trail '
                                                     'along cool mountain '
                                                     'water with mountain '
                                                     'vistas.'}],
    'lansdowne': [   {   'address': 'Upper Lansdowne, Pauri Garhwal, '
                                    'Uttarakhand',
                         'approx_cost': 0.0,
                         'category': 'Must Visit',
                         'closing_time': '19:00',
                         'description': 'Dramatic mountain cliff ridge in '
                                        'Lansdowne at 1,700m offering sweeping '
                                        'vistas of Garhwal Himalayan snow '
                                        'peaks including Chaukhamba and '
                                        'Trishul.',
                         'image_url': '/images/places/universal/stay.webp',
                         'is_hidden_gem': False,
                         'is_indoor': False,
                         'is_must_visit': True,
                         'latitude': 29.845,
                         'longitude': 78.692,
                         'name': 'Tip-in-Top Snow Peak Viewpoint',
                         'opening_time': '06:00',
                         'price_level': 'Free',
                         'rating': 4.8,
                         'review_count': 1420,
                         'slug': 'tip-in-top-lansdowne',
                         'tags': 'Viewpoint,Himalayan Panorama,Sunset,Ridge',
                         'why_vanvas_recommends': 'Breathtaking early morning '
                                                  'sunrise over the greater '
                                                  'Garhwal peaks.'},
                     {   'address': 'Kotdwar-Pauri Road, Lansdowne, '
                                    'Uttarakhand',
                         'approx_cost': 50.0,
                         'category': 'Nature & Trails',
                         'closing_time': '17:30',
                         'description': 'Serene man-made mountain lake '
                                        'maintained by the Garhwal Rifles, '
                                        'surrounded by tall oak and blue pine '
                                        'canopies with wooden bridges.',
                         'image_url': '/images/places/universal/stay.webp',
                         'is_hidden_gem': False,
                         'is_indoor': False,
                         'is_must_visit': True,
                         'latitude': 29.839,
                         'longitude': 78.685,
                         'name': 'Bhulla Tal Pine Forest Lake',
                         'opening_time': '08:00',
                         'price_level': '₹',
                         'rating': 4.7,
                         'review_count': 980,
                         'slug': 'bhulla-tal-lansdowne',
                         'tags': 'Lake,Pine Forest,Boating,Quiet Walks',
                         'why_vanvas_recommends': 'Tranquil walking path '
                                                  'through pine cones under '
                                                  'quiet cantonment skies.'}],
    'mathura-vrindavan': [   {   'address': 'Chatikara Road, Raman Reti, '
                                            'Vrindavan, UP',
                                 'approx_cost': 0.0,
                                 'category': 'Must Visit',
                                 'closing_time': '20:30',
                                 'description': 'Spectacular Italian white '
                                                'marble temple complex '
                                                'dedicated to Radha Krishna, '
                                                'known for intricate relief '
                                                'carvings and musical evening '
                                                'fountain light shows.',
                                 'image_url': '/images/places/universal/stay.webp',
                                 'is_hidden_gem': False,
                                 'is_indoor': False,
                                 'is_must_visit': True,
                                 'latitude': 27.5706,
                                 'longitude': 77.6599,
                                 'name': 'Prem Mandir Glowing White Marble '
                                         'Temple',
                                 'opening_time': '05:30',
                                 'price_level': 'Free',
                                 'rating': 4.9,
                                 'review_count': 3400,
                                 'slug': 'prem-mandir-vrindavan',
                                 'tags': 'Temple,Marble,Spiritual,Lighting,Vrindavan',
                                 'why_vanvas_recommends': 'Breathtaking '
                                                          'architectural '
                                                          'craftsmanship and '
                                                          'evening light '
                                                          'displays.'},
                             {   'address': 'Godowlia, Vrindavan, Mathura '
                                            'District, UP',
                                 'approx_cost': 0.0,
                                 'category': 'Culture & Heritage',
                                 'closing_time': '21:30',
                                 'description': 'The sacred spiritual '
                                                'heartbeat of Vrindavan, where '
                                                'devotees gather for darshan '
                                                'of Thakur Ji amidst '
                                                'devotional chants and flower '
                                                'offerings.',
                                 'image_url': '/images/places/universal/stay.webp',
                                 'is_hidden_gem': False,
                                 'is_indoor': True,
                                 'is_must_visit': True,
                                 'latitude': 27.5815,
                                 'longitude': 77.6985,
                                 'name': 'Banke Bihari Temple Sanctum',
                                 'opening_time': '07:45',
                                 'price_level': 'Free',
                                 'rating': 4.9,
                                 'review_count': 4200,
                                 'slug': 'banke-bihari-temple',
                                 'tags': 'Spiritual,Darshan,Vrindavan,Ancient '
                                         'Temple',
                                 'why_vanvas_recommends': 'The core devotional '
                                                          'shrine of the Braj '
                                                          'region.'}],
    'morni-hills': [   {   'address': 'Tikkar Taal, Morni Hills, Panchkula, '
                                      'Haryana',
                           'approx_cost': 0.0,
                           'category': 'Must Visit',
                           'closing_time': '19:00',
                           'description': 'Scenic twin lakes nestled in the '
                                          'Shivalik mountain hills of Morni, '
                                          'surrounded by pine woods and calm '
                                          'waters.',
                           'image_url': '/images/places/universal/stay.webp',
                           'is_hidden_gem': False,
                           'is_indoor': False,
                           'is_must_visit': True,
                           'latitude': 30.6908,
                           'longitude': 77.0869,
                           'name': 'Tikkar Taal Twin Lakes Sanctuary',
                           'opening_time': '06:00',
                           'price_level': 'Free',
                           'rating': 4.7,
                           'review_count': 1280,
                           'slug': 'tikkar-taal-morni',
                           'tags': 'Twin Lakes,Pine Woods,Boating,Quiet Hills',
                           'why_vanvas_recommends': 'Quiet mountain lake '
                                                    'escape surrounded by '
                                                    'green pine slopes.'},
                       {   'address': 'Morni Village, Panchkula, Haryana',
                           'approx_cost': 0.0,
                           'category': 'Nature & Trails',
                           'closing_time': '18:00',
                           'description': 'Historical 17th-century stone fort '
                                          'perched high on a scenic Shivalik '
                                          'mountain ridge overlooking deep '
                                          'green forest valleys.',
                           'image_url': '/images/places/universal/stay.webp',
                           'is_hidden_gem': True,
                           'is_indoor': False,
                           'is_must_visit': False,
                           'latitude': 30.695,
                           'longitude': 77.082,
                           'name': 'Morni Fort & Forest Hilltop Ridge',
                           'opening_time': '07:00',
                           'price_level': 'Free',
                           'rating': 4.6,
                           'review_count': 480,
                           'slug': 'morni-fort-ridge',
                           'tags': 'Fort,Panoramic Views,Ridge Walk,Nature',
                           'why_vanvas_recommends': 'Gentle forest ridge trail '
                                                    'with cool breeze and '
                                                    'expansive Shivalik '
                                                    'views.'}],
    'murthal': [   {   'address': '52.250 KM Stone, GT Road, Murthal, Haryana',
                       'approx_cost': 280.0,
                       'category': 'Local Food',
                       'closing_time': '23:59',
                       'description': 'Famous 24/7 highway stop along GT Road '
                                      'serving piping hot stuffed tandoori '
                                      'parathas with homemade white butter and '
                                      'rich sweet lassi.',
                       'image_url': '/images/places/universal/stay.webp',
                       'is_hidden_gem': False,
                       'is_indoor': True,
                       'is_must_visit': True,
                       'latitude': 29.0289,
                       'longitude': 77.0707,
                       'name': 'Amrik Sukhdev Legendary Dhaba',
                       'opening_time': '00:00',
                       'price_level': '₹₹',
                       'rating': 4.8,
                       'review_count': 2940,
                       'slug': 'amrik-sukhdev-dhaba',
                       'tags': 'Dhaba,Parathas,GT Road,Highway,Lassi',
                       'why_vanvas_recommends': 'The quintessential North '
                                                'Indian highway road-trip '
                                                'dining experience.'},
                   {   'address': 'GT Road, Murthal, Haryana',
                       'approx_cost': 350.0,
                       'category': 'Local Food',
                       'closing_time': '01:00',
                       'description': 'Theme-based heritage dhaba showcasing '
                                      'rural Punjabi cultural artifacts, clay '
                                      'tandoors, folk dances, and traditional '
                                      'thalis.',
                       'image_url': '/images/places/universal/stay.webp',
                       'is_hidden_gem': True,
                       'is_indoor': True,
                       'is_must_visit': False,
                       'latitude': 29.035,
                       'longitude': 77.072,
                       'name': 'Haveli GT Road Heritage Dhaba',
                       'opening_time': '06:00',
                       'price_level': '₹₹',
                       'rating': 4.7,
                       'review_count': 1420,
                       'slug': 'haveli-gt-road',
                       'tags': 'Heritage,Punjabi Food,Family,Dhaba',
                       'why_vanvas_recommends': 'Vibrant folk decor and '
                                                'traditional culinary heritage '
                                                'on NH-44.'}],
    'neemrana': [   {   'address': '15th Century Fort Palace, Neemrana, Alwar, '
                                   'Rajasthan',
                        'approx_cost': 1500.0,
                        'category': 'Must Visit',
                        'closing_time': '18:00',
                        'description': 'Historic 14-tiered fortress palace '
                                       'built into the Aravalli cliffs in 1464 '
                                       'AD, offering hanging gardens, '
                                       'courtyard pools, and panoramic valley '
                                       'views.',
                        'image_url': '/images/places/universal/stay.webp',
                        'is_hidden_gem': False,
                        'is_indoor': False,
                        'is_must_visit': True,
                        'latitude': 27.9942,
                        'longitude': 76.3869,
                        'name': 'Neemrana Fort Palace 15th-Century Ramparts',
                        'opening_time': '09:00',
                        'price_level': '₹₹₹',
                        'rating': 4.8,
                        'review_count': 2800,
                        'slug': 'neemrana-fort-palace-ramparts',
                        'tags': 'Fort,Palace,Heritage,Aravalli,Architecture',
                        'why_vanvas_recommends': 'Majestic stepped '
                                                 'architecture and historical '
                                                 'heritage atop the Aravalli '
                                                 'hills.'},
                    {   'address': 'Neemrana Fort-Palace, Alwar, Rajasthan',
                        'approx_cost': 1800.0,
                        'category': 'Adventure',
                        'closing_time': '17:00',
                        'description': 'Thrilling 5-line aerial zip tour '
                                       'gliding high above the medieval fort '
                                       'battlements and rocky Aravalli ridges.',
                        'image_url': '/images/places/universal/stay.webp',
                        'is_hidden_gem': True,
                        'is_indoor': False,
                        'is_must_visit': False,
                        'latitude': 27.995,
                        'longitude': 76.3875,
                        'name': 'Flying Fox Neemrana Zip-Line',
                        'opening_time': '09:30',
                        'price_level': '₹₹₹',
                        'rating': 4.8,
                        'review_count': 890,
                        'slug': 'flying-fox-neemrana',
                        'tags': 'Adventure,Zip-Line,Aravalli,Views',
                        'why_vanvas_recommends': "Unbeatable bird's-eye "
                                                 'perspective of the '
                                                 '15th-century fortress.'}],
    'sariska-bhangarh': [   {   'address': 'Sariska Tiger Reserve, Alwar, '
                                           'Rajasthan',
                                'approx_cost': 950.0,
                                'category': 'Must Visit',
                                'closing_time': '18:00',
                                'description': 'Famous dry deciduous tiger '
                                               'reserve in the Aravalli hills '
                                               'home to Royal Bengal tigers, '
                                               'leopards, sambar deer, and '
                                               'ancient ruins.',
                                'image_url': '/images/places/universal/stay.webp',
                                'is_hidden_gem': False,
                                'is_indoor': False,
                                'is_must_visit': True,
                                'latitude': 27.3292,
                                'longitude': 76.4389,
                                'name': 'Sariska National Park Tiger Safari',
                                'opening_time': '06:00',
                                'price_level': '₹₹₹',
                                'rating': 4.7,
                                'review_count': 2100,
                                'slug': 'sariska-national-park-safari',
                                'tags': 'Wildlife,Safari,Tigers,National '
                                        'Park,Forest',
                                'why_vanvas_recommends': 'Thrilling jungle '
                                                         'safari tracking wild '
                                                         'predators in ancient '
                                                         'Aravalli valleys.'},
                            {   'address': 'Bhangarh, Rajgarh Tehsil, Alwar, '
                                           'Rajasthan',
                                'approx_cost': 40.0,
                                'category': 'Culture & Heritage',
                                'closing_time': '18:00',
                                'description': '17th-century medieval '
                                               'fortified town ruins featuring '
                                               'intact stone temples, palace '
                                               'ramparts, and banyan trees '
                                               'against mountain cliffs.',
                                'image_url': '/images/places/universal/stay.webp',
                                'is_hidden_gem': False,
                                'is_indoor': False,
                                'is_must_visit': True,
                                'latitude': 27.0965,
                                'longitude': 76.2864,
                                'name': 'Bhangarh Fort Medieval Ruins',
                                'opening_time': '06:00',
                                'price_level': '₹',
                                'rating': 4.8,
                                'review_count': 3100,
                                'slug': 'bhangarh-fort-ruins',
                                'tags': 'Historic '
                                        'Ruins,Archaeology,Mystery,Aravalli',
                                'why_vanvas_recommends': 'Dramatic stone town '
                                                         'architecture '
                                                         'preserved inside a '
                                                         'rugged mountain '
                                                         'valley.'}]}

ADDITIONAL_HOTELS_BY_DEST = {   'agra': [   {   'address': 'Fatehabad Road, Tajganj, Agra, UP',
                    'amenities': 'Rooftop Taj View,Mughal Courtyard,Swimming '
                                 'Pool,Spa',
                    'badge': 'Taj View Sanctuary',
                    'booking_url': 'https://booking.vanvas.com/taj-view-retreat',
                    'check_in_time': '02:00 PM',
                    'check_out_time': '12:00 PM',
                    'hotel_style': 'Heritage / Taj View',
                    'image_url': '/images/places/universal/heritage.webp',
                    'latitude': 27.165,
                    'longitude': 78.038,
                    'name': 'Taj View Heritage Courtyard Retreat',
                    'price_per_night': 5500.0,
                    'rating': 4.9}],
    'alwar-siliserh': [   {   'address': 'Siliserh Lake, Alwar, Rajasthan',
                              'amenities': 'Lakefront Rooms,Boating '
                                           'Deck,Aravalli Balcony,Restaurant',
                              'badge': 'Lake Palace Stay',
                              'booking_url': 'https://booking.vanvas.com/siliserh-palace',
                              'check_in_time': '12:00 PM',
                              'check_out_time': '11:00 AM',
                              'hotel_style': 'Heritage / Lakefront Palace',
                              'image_url': '/images/places/universal/resort.webp',
                              'latitude': 27.5342,
                              'longitude': 76.5411,
                              'name': 'RTDC Hotel Lake Palace Siliserh',
                              'price_per_night': 3500.0,
                              'rating': 4.7}],
    'chandigarh': [   {   'address': 'Sector 3, Near Sukhna Lake, Chandigarh',
                          'amenities': 'Lake Access,Courtyard '
                                       'Garden,Terrace,Modernist Design',
                          'badge': 'Modernist Villa',
                          'booking_url': 'https://booking.vanvas.com/sukhna-villa',
                          'check_in_time': '01:00 PM',
                          'check_out_time': '11:00 AM',
                          'hotel_style': 'Boutique / Modernist Villa',
                          'image_url': '/images/places/universal/boutique.webp',
                          'latitude': 30.738,
                          'longitude': 76.812,
                          'name': 'Sukhna Lake Modernist Boutique Villa',
                          'price_per_night': 3800.0,
                          'rating': 4.8}],
    'damdama-sohna': [   {   'address': 'Damdama Lake, Near Sohna, Gurugram, '
                                        'Haryana',
                             'amenities': 'Lake Views,Nature Trails,Outdoor '
                                          'Pool,Spa,Archery',
                             'badge': 'Lakeside Resort',
                             'booking_url': 'https://booking.vanvas.com/damdama-gateway',
                             'check_in_time': '02:00 PM',
                             'check_out_time': '11:00 AM',
                             'hotel_style': 'Resort / Lakeside Sanctuary',
                             'image_url': '/images/places/universal/stay.webp',
                             'latitude': 28.309,
                             'longitude': 77.064,
                             'name': 'The Gateway Resort Damdama Lake',
                             'price_per_night': 6500.0,
                             'rating': 4.8}],
    'dehradun': [   {   'address': 'Old Rajpur Road, Dehradun, Uttarakhand',
                        'amenities': 'Foothill Balcony,Artisan '
                                     'Bakery,High-Speed WiFi,Garden',
                        'badge': 'Rajpur Heritage Retreat',
                        'booking_url': 'https://booking.vanvas.com/rajpur-heritage',
                        'check_in_time': '01:00 PM',
                        'check_out_time': '11:00 AM',
                        'hotel_style': 'Heritage / Foothill Boutique',
                        'image_url': '/images/places/universal/boutique.webp',
                        'latitude': 30.355,
                        'longitude': 78.062,
                        'name': 'The Rajpur Heritage Retreat & Bakery',
                        'price_per_night': 3400.0,
                        'rating': 4.8}],
    'kainchi-dham': [   {   'address': 'Bhowali-Kainchi Road, Nainital '
                                       'District, UK',
                            'amenities': 'Riverside Balcony,Satvik '
                                         'Meals,Meditation Hall,Valley View',
                            'badge': 'Ashram Valley Homestay',
                            'booking_url': 'https://booking.vanvas.com/kainchi-retreat',
                            'check_in_time': '12:00 PM',
                            'check_out_time': '10:00 AM',
                            'hotel_style': 'Homestay / Kumaon Valley',
                            'image_url': '/images/places/universal/homestay.webp',
                            'latitude': 29.423,
                            'longitude': 79.515,
                            'name': 'The Kumaon Riverside Ashram Retreat',
                            'price_per_night': 1800.0,
                            'rating': 4.9}],
    'lansdowne': [   {   'address': 'Tip-in-Top Road, Lansdowne, Uttarakhand',
                         'amenities': 'Blue Pine Balcony,Snow Peak '
                                      'Views,Garhwali Meals,Bonfire',
                         'badge': 'Blue Pine Lodge',
                         'booking_url': 'https://booking.vanvas.com/lansdowne-pine-lodge',
                         'check_in_time': '12:00 PM',
                         'check_out_time': '10:00 AM',
                         'hotel_style': 'Homestay / Wooden Pine Cabin',
                         'image_url': '/images/places/universal/homestay.webp',
                         'latitude': 29.842,
                         'longitude': 78.69,
                         'name': 'Lansdowne Blue Pine Forest Lodge',
                         'price_per_night': 2600.0,
                         'rating': 4.8}],
    'mathura-vrindavan': [   {   'address': 'Raman Reti Road, Vrindavan, '
                                            'Mathura, UP',
                                 'amenities': 'Satvik Dining,Spiritual '
                                              'Library,Temple Shuttle,WiFi',
                                 'badge': 'Temple Heritage Stay',
                                 'booking_url': 'https://booking.vanvas.com/braj-heritage',
                                 'check_in_time': '01:00 PM',
                                 'check_out_time': '11:00 AM',
                                 'hotel_style': 'Homestay / Temple Sanctuary',
                                 'image_url': '/images/places/universal/stay.webp',
                                 'latitude': 27.575,
                                 'longitude': 77.665,
                                 'name': 'Braj Heritage Temple Stay & Retreat',
                                 'price_per_night': 2400.0,
                                 'rating': 4.8}],
    'morni-hills': [   {   'address': 'Tikkar Taal Road, Morni Hills, '
                                      'Panchkula, Haryana',
                           'amenities': 'Pine Forest Balcony,Tikkar Taal '
                                        'View,Bonfire,Restaurant',
                           'badge': 'Pine Forest Resort',
                           'booking_url': 'https://booking.vanvas.com/morni-heights',
                           'check_in_time': '12:00 PM',
                           'check_out_time': '10:00 AM',
                           'hotel_style': 'Resort / Pine Mountain View',
                           'image_url': '/images/places/universal/resort.webp',
                           'latitude': 30.692,
                           'longitude': 77.085,
                           'name': 'Morni Heights Forest & Lake Resort',
                           'price_per_night': 2800.0,
                           'rating': 4.7}],
    'murthal': [   {   'address': 'GT Road, Murthal, Sonipat, Haryana',
                       'amenities': 'Heritage Courtyard,24/7 Dining,Free '
                                    'Parking,WiFi',
                       'badge': 'Highway Heritage Resort',
                       'booking_url': 'https://booking.vanvas.com/grand-haveli-murthal',
                       'check_in_time': '01:00 PM',
                       'check_out_time': '11:00 AM',
                       'hotel_style': 'Resort / Heritage',
                       'image_url': '/images/places/universal/stay.webp',
                       'latitude': 29.036,
                       'longitude': 77.073,
                       'name': 'Grand Haveli Resort & Highway Sanctuary',
                       'price_per_night': 3200.0,
                       'rating': 4.8}],
    'neemrana': [   {   'address': '15th Century Fort Palace, Neemrana, '
                                   'Rajasthan',
                        'amenities': 'Stepped Courtyards,Pools,Zip-Line,Royal '
                                     'Dining,Spa',
                        'badge': 'Royal Fortress Palace',
                        'booking_url': 'https://booking.vanvas.com/neemrana-fort',
                        'check_in_time': '02:00 PM',
                        'check_out_time': '11:00 AM',
                        'hotel_style': 'Heritage / 15th Century Palace',
                        'image_url': '/images/places/universal/heritage.webp',
                        'latitude': 27.9942,
                        'longitude': 76.3869,
                        'name': 'Neemrana Fort-Palace Heritage Luxury',
                        'price_per_night': 9500.0,
                        'rating': 4.9}],
    'sariska-bhangarh': [   {   'address': 'Alwar-Jaipur Road, Sariska, '
                                           'Rajasthan',
                                'amenities': 'Safari Desk,Campfire Lounge,Folk '
                                             'Music,Pool',
                                'badge': 'Wilderness Camp',
                                'booking_url': 'https://booking.vanvas.com/sariska-tiger-camp',
                                'check_in_time': '01:00 PM',
                                'check_out_time': '11:00 AM',
                                'hotel_style': 'Resort / Wilderness Camp',
                                'image_url': '/images/places/universal/resort.webp',
                                'latitude': 27.332,
                                'longitude': 76.442,
                                'name': 'Sariska Tiger Camp Wilderness Resort',
                                'price_per_night': 4200.0,
                                'rating': 4.8}]}

ADDITIONAL_RENTALS_BY_DEST = {   'agra': [   {   'deposit_amount': 1000.0,
                    'image_url': '/images/vehicles/electric_scooter.jpg',
                    'latitude': 27.172,
                    'location': 'Taj East Gate Road, Agra',
                    'longitude': 78.025,
                    'opening_hours': '06:30 AM - 09:00 PM',
                    'price_per_day': 500.0,
                    'provider_name': 'Agra Heritage Moto & EV Rentals',
                    'rating': 4.8,
                    'vehicle_name': 'Ather 450X Taj Explorer EV',
                    'vehicle_type': 'Electric Scooter'}],
    'alwar-siliserh': [   {   'deposit_amount': 1000.0,
                              'image_url': '/images/vehicles/automatic_scooter.jpg',
                              'latitude': 27.56,
                              'location': 'Church Road, Alwar, Rajasthan',
                              'longitude': 76.6,
                              'opening_hours': '08:00 AM - 08:30 PM',
                              'price_per_day': 450.0,
                              'provider_name': 'Alwar City Bike & Scooter Hub',
                              'rating': 4.8,
                              'vehicle_name': 'Honda Activa 6G Palace Edition',
                              'vehicle_type': 'Scooter'}],
    'chandigarh': [   {   'deposit_amount': 1000.0,
                          'image_url': '/images/vehicles/electric_scooter.jpg',
                          'latitude': 30.735,
                          'location': 'Sector 17, Chandigarh',
                          'longitude': 76.78,
                          'opening_hours': '07:00 AM - 09:30 PM',
                          'price_per_day': 400.0,
                          'provider_name': 'Chandigarh Green Mobility EV '
                                           'Scooters',
                          'rating': 4.9,
                          'vehicle_name': 'Ather 450X Shivalik EV',
                          'vehicle_type': 'Electric Scooter'}],
    'damdama-sohna': [   {   'deposit_amount': 1000.0,
                             'image_url': '/images/vehicles/automatic_scooter.jpg',
                             'latitude': 28.25,
                             'location': 'Sohna Road, Gurugram',
                             'longitude': 77.06,
                             'opening_hours': '07:30 AM - 08:30 PM',
                             'price_per_day': 450.0,
                             'provider_name': 'Sohna Aravalli Wheels',
                             'rating': 4.7,
                             'vehicle_name': 'Honda Activa 6G Lakeside Edition',
                             'vehicle_type': 'Scooter'}],
    'dehradun': [   {   'deposit_amount': 1000.0,
                        'image_url': '/images/vehicles/automatic_scooter.jpg',
                        'latitude': 30.32,
                        'location': 'Rajpur Road, Dehradun, UK',
                        'longitude': 78.04,
                        'opening_hours': '07:30 AM - 09:00 PM',
                        'price_per_day': 450.0,
                        'provider_name': 'Doon Valley Moto Rentals',
                        'rating': 4.9,
                        'vehicle_name': 'Honda Activa 6G Foothill Cruiser',
                        'vehicle_type': 'Scooter'}],
    'kainchi-dham': [   {   'deposit_amount': 1000.0,
                            'image_url': '/images/vehicles/automatic_scooter.jpg',
                            'latitude': 29.385,
                            'location': 'Bhowali Bus Stand, Near Kainchi Dham',
                            'longitude': 79.52,
                            'opening_hours': '07:00 AM - 08:30 PM',
                            'price_per_day': 500.0,
                            'provider_name': 'Bhowali-Kainchi Mountain Wheels',
                            'rating': 4.8,
                            'vehicle_name': 'Honda Activa 6G Kumaon Cruiser',
                            'vehicle_type': 'Scooter'}],
    'lansdowne': [   {   'deposit_amount': 1000.0,
                         'image_url': '/images/vehicles/automatic_scooter.jpg',
                         'latitude': 29.838,
                         'location': 'Sadar Bazaar, Lansdowne, UK',
                         'longitude': 78.686,
                         'opening_hours': '07:30 AM - 08:30 PM',
                         'price_per_day': 500.0,
                         'provider_name': 'Garhwal Pines Scooter & Bike '
                                          'Rentals',
                         'rating': 4.8,
                         'vehicle_name': 'Honda Activa 6G Pine Explorer',
                         'vehicle_type': 'Scooter'}],
    'mathura-vrindavan': [   {   'deposit_amount': 1000.0,
                                 'image_url': '/images/vehicles/automatic_scooter.jpg',
                                 'latitude': 27.572,
                                 'location': 'Chatikara Road, Vrindavan',
                                 'longitude': 77.661,
                                 'opening_hours': '06:00 AM - 09:30 PM',
                                 'price_per_day': 450.0,
                                 'provider_name': 'Braj Yatra Scooter Rentals',
                                 'rating': 4.9,
                                 'vehicle_name': 'Honda Activa 6G Vrindavan '
                                                 'Edition',
                                 'vehicle_type': 'Scooter'}],
    'morni-hills': [   {   'deposit_amount': 1000.0,
                           'image_url': '/images/vehicles/automatic_scooter.jpg',
                           'latitude': 30.69,
                           'location': 'Morni Hills Main Stand, Panchkula',
                           'longitude': 77.084,
                           'opening_hours': '08:00 AM - 08:00 PM',
                           'price_per_day': 500.0,
                           'provider_name': 'Shivalik Hills Moto Hub',
                           'rating': 4.7,
                           'vehicle_name': 'Honda Activa 6G Hill Edition',
                           'vehicle_type': 'Scooter'}],
    'murthal': [   {   'deposit_amount': 2000.0,
                       'image_url': '/images/vehicles/classic_bullet.jpg',
                       'latitude': 29.028,
                       'location': 'GT Road, Murthal, Haryana',
                       'longitude': 77.07,
                       'opening_hours': '07:00 AM - 10:00 PM',
                       'price_per_day': 1200.0,
                       'provider_name': 'GT Road Highway Cruisers',
                       'rating': 4.8,
                       'vehicle_name': 'Classic 350 Highway Cruiser',
                       'vehicle_type': 'Royal Enfield Classic 350'}],
    'neemrana': [   {   'deposit_amount': 2000.0,
                        'image_url': '/images/vehicles/classic_bullet.jpg',
                        'latitude': 27.99,
                        'location': 'NH-48, Neemrana, Rajasthan',
                        'longitude': 76.382,
                        'opening_hours': '08:00 AM - 08:30 PM',
                        'price_per_day': 1100.0,
                        'provider_name': 'Aravalli Highway Moto Hub',
                        'rating': 4.8,
                        'vehicle_name': 'Classic 350 Fort Explorer',
                        'vehicle_type': 'Royal Enfield Classic 350'}],
    'sariska-bhangarh': [   {   'deposit_amount': 2500.0,
                                'image_url': '/images/vehicles/adventure_motorcycle.jpg',
                                'latitude': 27.33,
                                'location': 'Sariska Gate, Alwar, Rajasthan',
                                'longitude': 76.44,
                                'opening_hours': '06:30 AM - 08:00 PM',
                                'price_per_day': 1300.0,
                                'provider_name': 'Sariska Wilderness Safari '
                                                 'Bikes',
                                'rating': 4.9,
                                'vehicle_name': 'Himalayan 450 Safari Explorer',
                                'vehicle_type': 'Royal Enfield Himalayan 450'}]}
